import os
import pandas as pd
from common import is_abstract
from tqdm import tqdm


def evaluate_template_level(dataset,
                            df_groundtruth,
                            df_parsedresult,
...
def evaluate_template_level_lstm(dataset,
                                 df_groundtruth,
                                 df_parsedresult,
                                 filter_templates=None):
    """
    ...
    for identified_template, group in tqdm(grouped_df):
        ...
        if len(corr_oracle_templates) == 1 and correct_lstm(
                identified_template,
                list(corr_oracle_templates)[0]):
            if (filter_templates is None) or (list(corr_oracle_templates)[0]
                                              in filter_templates):
                correct_parsing_templates += 1
        # else:
        # print(corr_oracle_templates, set([identified_template]))
